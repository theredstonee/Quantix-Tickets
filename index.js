// --- index.js | Ticket‑Bot v6.3.2 (Channel Rename Fix bei Prioritätswechsel) ---
// Nur relevante Änderung: renameChannelIfNeeded wird NACH dem Schreiben der neuen Priorität
// UND bevor weitere Antworten gesendet werden sicher aufgerufen.
// Rest deines zuletzt geposteten Codes bleibt unverändert.
...
let luxonAvailable = true; try{ require.resolve('luxon'); } catch { luxonAvailable=false; }
...
async function renameChannelIfNeeded(channel, ticket){
  const desired = buildChannelName(ticket.id, ticket.priority||0);
  if(channel.name !== desired){
    try { await channel.setName(desired); } catch(err){ /* ignore */ }
  }
}
...
async function logEvent(guild, content, embed=null){
  ...
  try {
    const ch = await guild.channels.fetch(cfg.logChannelId); if(!ch) return;
    await ch.send({ content, embeds: embed? [embed]: undefined });
...
async function createTranscript(channel, ticket){
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Transcript Ticket ${ticket.id}</title><style>body{font-family:Arial, sans-serif; background:#111; color:#eee;} .msg{margin:4px 0;} .time{color:#888;font-size:11px;margin-right:6px;} .author{color:#4ea1ff;font-weight:bold;margin-right:4px;} .attach{color:#ffa500; font-size:11px; display:block; margin-left:2rem;}</style></head><body><h1>Transcript Ticket ${ticket.id}</h1><p>Channel: ${channel.name}<br>Erstellt: ${new Date(ticket.timestamp).toISOString()}<br>Nachrichten: ${messages.length}</p><hr>${messages.map(m=>{const atts=m.attachments.size?[...m.attachments.values()].map(a=>`<span class="attach">📎 <a href="${a.url}">${a.name}</a></span>`).join(''):'';return `<div class="msg"><span class="time">${new Date(m.createdTimestamp).toISOString()}</span><span class="author">${m.author?.tag||m.author?.id}</span><span class="content">${(m.content||'').replace(/</g,'&lt;')}</span>${atts}</div>`;}).join('\n')}</body></html>`;
  const transcriptTxtPath  = path.join(__dirname, `transcript_${ticket.id}.txt`);
  const transcriptHtmlPath = path.join(__dirname, `transcript_${ticket.id}.html`);
  fs.writeFileSync(transcriptTxtPath, plainText);
  fs.writeFileSync(transcriptHtmlPath, html);
  return { txt: new AttachmentBuilder(transcriptTxtPath), html: new AttachmentBuilder(transcriptHtmlPath) };
}
    ...
    if(i.isButton()){
      ...
      switch(i.customId){
        ...
        case 'priority_up': {
          ...
          safeWrite(TICKETS_PATH, log); // sofort speichern
          await renameChannelIfNeeded(i.channel, ticket); // <-- WICHTIG: direkt nach Änderung
          await updatePriorityEmbed(i, ticket, 'hoch');
          break;
        }
        case 'priority_down': {
          ticket.priority = Math.max(0, (ticket.priority||0)-1);
          safeWrite(TICKETS_PATH, log);
          await renameChannelIfNeeded(i.channel, ticket); // <-- WICHTIG
          await updatePriorityEmbed(i, ticket, 'herab');
          break;
        }
        case 'add_user': {
...
/* ================= Priority Embed Update ================= */
async function updatePriorityEmbed(interaction, ticket, dir){
  try {
    const state = PRIORITY_STATES[ticket.priority||0];
    // Suche die erste Embed-Nachricht im Channel (Ticket-Eröffnungsnachricht)
    ...
    console.error('Priority Update Fehler:', err);
    if(!interaction.replied && !interaction.deferred) interaction.reply({ephemeral:true,content:'Fehler beim Prioritäts-Update'});
  }
}
